...
begin
    ...
    pbeam = BunchedBeam(PROTON, 0.688e11, 275e9, num_particles, [11.3e-9, 1e-9, 3.7e-2])  #[11.3e-9, 1e-9, 3.7e-2] #3.7e-2 -> 6cm
...
for i in 1:turns
    ...
    if i < turns_rec
        ...
        for j in 1:nSC
            ...
            sc_BE.sigma_xi_SC[i,j], sc_BE.sigma_yi_SC[i,j] = SC_kick!(sc_BE, pbeam)
...
plot(records.sx[turns_rec+1:turns], label = L"$σ_x$", xlabel="# Turns")
plot(records.ex[turns_rec+1:turns], label = L"$ε_x$", xlabel="# Turns")
plot(records.sz[turns_rec+1:turns], label = L"$σ_z$", xlabel="# Turns")
plot(records.cx[turns_rec+1:turns], label = L"$σ_x$ centroid", xlabel="# Turns")
